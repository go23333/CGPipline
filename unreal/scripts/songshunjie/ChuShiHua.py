...
class mw(QtWidgets.QWidget, MFieldMixin):
    ...
    def createDirectory(self):
        ...
        for sc_list_i in sc_list:
            cam_list=[]
            for cam_i in self.cam:
                if cam_i.split('_',2)[1]==sc_list_i:
            ...
            for cam_name in cam_dict[sc_name]:
                for file_class_name in self.flie_class:
                    if file_class_name=='Lighting':
                        flie_name='%s_lt'%(cam_name)
                        unreal.AssetToolsHelpers.get_asset_tools().create_asset(asset_name='%s'%(cam_name),package_path='/Game/Shots/%s/%s/%s/%s'%(ep,file_class_name,sc_name,flie_name),asset_class=unreal.World,factory=unreal.WorldFactory())
                        unreal.AssetToolsHelpers.get_asset_tools().create_asset(asset_name='%s'%(flie_name),package_path='/Game/Shots/%s/%s/%s/%s'%(ep,file_class_name,sc_name,flie_name),asset_class=unreal.LevelSequence,factory=unreal.LevelSequenceFactoryNew())
                    if file_class_name=='Animation':
                        flie_name='%s_an'%(cam_name)
                        unreal.AssetToolsHelpers.get_asset_tools().create_asset(asset_name='%s'%(flie_name),package_path='/Game/Shots/%s/%s/%s/%s'%(ep,file_class_name,sc_name,flie_name),asset_class=unreal.LevelSequence,factory=unreal.LevelSequenceFactoryNew())
                    if file_class_name=='Cache':
                        flie_name='%s_cache'%(cam_name)
                    if file_class_name=='VFX':
                        flie_name='%s_vfx'%(cam_name)
                    if file_class_name=='Modify':
                        flie_name='%s_modify'%(cam_name)
