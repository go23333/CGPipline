...
class BlindWatermark():
    ...
    def calc_sim(sig1,sig2s):
        max_sim =0 
        for  sig2 in sig2s:
            match_cnt = np.sum(np.equal(np.array(sig1,dtype=np.int32),np.array(sig2,dtype=np.int32)))
...
class DCT_watermark(BlindWatermark):
    ...
    #如果嵌入到低频区域，问题失真更大。
    def inner_embed(self,B,signature):
        sig_size=np.int32(np.sqrt(len(signature)))
        size = self.size
    ...
    def inner_extract(self,B,signature):
        ...
        for x,y in embed_pos:
            ext_sig = np.zeros(len(signature),dtype=np.int32)
