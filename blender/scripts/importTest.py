...
def createMaterialNode(materila,data):
    ...
    if data["normalPath"] != None:
        ...
        links.new(normalMapNode.outputs[0],bsdf.inputs[19])
    ...
    if data["type"] == "PBR":
        ...
        if data["roughnessPath"] != None:
            ...
            links.new(splitXYZNode.outputs[1],bsdf.inputs[7])
        ...
        if data["metallicPath"] != None:
            ...
            links.new(splitXYZNode.outputs[2],bsdf.inputs[4])
    ...
    # 当类型为传统模式时
        ...
        if data["glossinessPath"] != None:
            ...
            links.new(invertColorNode.outputs[0],bsdf.inputs[7])
        ...
        if data["f0Path"] != None:
            ...
            if useUDIM:
                image.source = "TILED"
            f0Node.image = image
            links.new(f0Node.outputs[0],bsdf.inputs[4])
...
def doImport(context, filepath, use_some_setting):
...
class CustomImport(Operator, ImportHelper):
    """This appears in the tooltip of the operator and in the generated docs"""
    bl_idname = "customimport.doimport"  # important since its how bpy.ops.import_test.some_data is constructed
    ...
    def execute(self, context):
        return doImport(context, self.filepath, self.use_setting)


def menu_func_import(self, context):
    self.layout.operator(CustomImport.bl_idname, text="Custom Import")


def register():
    bpy.utils.register_class(CustomImport)
    bpy.types.TOPBAR_MT_file_import.append(menu_func_import)

def unregister():
    bpy.utils.unregister_class(CustomImport)
...
def test():
    try:
        ...
        pass
    register()
    bpy.ops.customimport.doimport('INVOKE_DEFAULT')
