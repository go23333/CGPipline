...
class NP():			
	...
	def UI(self):
		...
		# Modiy selection
		####################
		cmds.frameLayout( label=u"编辑选择",w=160, cll=True,cl=0,ec=partial(frameCollapseChanged, str(modelLayout)), cc=partial(frameCollapseChanged, str(modelLayout)),bgc=frameColor)
		cmds.separator(h=1)
		cmds.rowColumnLayout(numberOfColumns=4,columnSpacing=[(1, 8),(2, 2),(3, 5),(4, 2)])
		cmds.button(label=u"循环边扩展",w=85,h=buttonHeight, c=lambda x: self.growLoop(),bgc=plusClr)
		cmds.button(label=u"循环边缩减", w=85,h=buttonHeight, c=lambda x: self.shrinkLoop(),bgc=minClr)
		cmds.button(label=u"并排便扩展",w=85,h=buttonHeight, c=lambda x: self.growRing(),bgc=plusClr)
		cmds.button(label=u"并排便缩减", w=85,h=buttonHeight, c=lambda x: self.shrinkRing(),bgc=minClr)
		cmds.setParent( '..' )
		...
		cmds.button(label=u"间隔循环",w=winWidth/2-30,h=buttonHeight, c=lambda x: self.dotLoop(),bgc=plusClr)
		cmds.button(label=u"间隔并排", w=winWidth/2-36,h=buttonHeight, c=lambda x: self.dotRing(),bgc=minClr)
		cmds.setParent( '..' )
		cmds.setParent( '..' )
		
		cmds.separator(h=1)
		cmds.rowColumnLayout(numberOfColumns=4,columnSpacing=[(1, 8),(2, 2),(3, 5),(4, 2)])
		cmds.button(label=u"选择硬边", w=85,h=buttonHeight, c=lambda x: self.hardEdge(),bgc=buttonColor)
		cmds.button(label=u"选择UV边界",w=85,h=buttonHeight, c=lambda x: self.uvEdge(),bgc=buttonColor)
		...
		cmds.separator(h=1)
		cmds.rowColumnLayout(numberOfColumns=4,columnSpacing=[(1, 8),(2, 2),(3, 5),(4, 2)])
		cmds.button(label=u'合并网格',h=buttonHeight,w=85,c=lambda x: self.CombineClean(),bgc=plusClr)
		cmds.button(label=u'分离网格',h=buttonHeight,w=85,c=lambda x: self.detatchClean(),bgc=minClr)
		cmds.button(label=u'UniConnect',h=buttonHeight,w=85,c=lambda x: self.UniConnector(),bgc=plusClr)
		cmds.button(label=u'UniRemove',h=buttonHeight,w=85,c=lambda x: self.UniRemover(),bgc=minClr)
		...
		cmds.separator(h=1)
		cmds.rowColumnLayout(numberOfColumns=4,columnSpacing=[(1, 8),(2, 2),(3, 5),(4, 2)])
		cmds.button(label=u'剧中枢轴',h=buttonHeight,w=85,c=lambda x: self.basepivot(),bgc=buttonColor)
		cmds.button(label=u'世界中心枢轴',h=buttonHeight,w=85,c=lambda x: self.worldPivot(),bgc=buttonColor)
		...
		cmds.button(label=u'边角旋转+45°',h=buttonHeight,w=winWidth/2-15,c=lambda *args: self.corner45(True),bgc=plusClr)
		cmds.button(label=u'边角旋转-45°',h=buttonHeight,w=winWidth/2-10,c=lambda *args: self.corner45(False),bgc=minClr)
		cmds.setParent( '..' )
		cmds.separator(h=1)
