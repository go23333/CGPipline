...
class win():
	...
	def UI1(self):
		...
		cmds.textFieldGrp('start',label=u'设置起始帧',text='%s'%self.start,cal=(1,'left'),cw2=(100,50))
		cmds.textFieldGrp('end',label=u'设置结束帧',text='%s'%self.end,cal=(1,'left'),cw2=(100,50))
		cmds.text( label=u'选择需要BlendShape的对象后点击执行', align='left'  )
		cmds.button( label=u'执行',command=self.bs)
